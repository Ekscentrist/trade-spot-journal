...
const logger = new Logger('BitgetTicker');
...
export function instIdToBitgetSymbol(instId: string): string {
  return instId.replace(/-/g, '').toUpperCase();
}

async function fetchTickerLast(instId: string): Promise<number | null> {
  try {
    const symbol = instIdToBitgetSymbol(instId);
    const url = `https://api.bitget.com/api/v2/spot/market/tickers?symbol=${encodeURIComponent(symbol)}`;
    ...
    if (json.code !== '00000' || !json.data?.[0]?.lastPr) {
      logger.warn(`Ticker bad response for ${instId}: ${json.code}`);
      return null;
    }
    const last = Number(json.data[0].lastPr);
...
export async function getBitgetLastPrices(
  instIds: string[],
